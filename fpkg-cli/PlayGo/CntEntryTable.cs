...
internal sealed class CntEntry
{
    ...
    /// <summary>
    /// An independent copy, payload arrays included. <see cref="CntReseal.Seal"/> works on copies
    /// so that a pre-flight reseal cannot leave its layout assignments on the caller's table.
    /// </summary>
    internal CntEntry Clone() => new()
    {
        Id = Id, NameTableOffset = NameTableOffset, Flags1 = Flags1, Flags2 = Flags2,
        DataOffset = DataOffset, DataSize = DataSize, Name = Name,
        Payload = (byte[])Payload.Clone(), StoredPayload = (byte[])StoredPayload.Clone(),
    };
