...
internal static class CntReseal
{
    ...
    /// </summary>
    /// <summary>
    /// The pre-flight gate: reseal <paramref name="cnt"/> with nothing changed and report the
    /// first byte that does not come back identical, or null when it is a fixed point.
    ///
    /// <para>
    /// A package that fails this has stored digests that already disagree with its own bytes.
    /// The library's own verifier does NOT catch that — it recomputes the chain the same way the
    /// resealer does, so it agrees with whatever is stored. Repairing such a package silently
    /// corrects its header signature on top of whatever else the repair does, which makes the
    /// result impossible to reason about.
    /// </para>
    /// <para>
    /// Seal assigns DataOffset/DataSize on the entries it is given, so callers must pass a table
    /// they are not going to reuse for the real repair.
    /// </para>
    /// </summary>
    internal static string? FixedPointProblem(byte[] cnt, IReadOnlyList<CntEntry> physical,
                                              string contentId, string passcode)
    {
        var resealed = Seal(cnt, physical, contentId, passcode);
        if (resealed.LongLength != cnt.LongLength)
            return $"resealing changes the CNT length ({cnt.LongLength:N0} -> {resealed.LongLength:N0}).";
        for (long i = 0; i < cnt.LongLength; i++)
            if (cnt[i] != resealed[i])
                return $"resealing this package with nothing changed does not reproduce it: the " +
                       $"first difference is at CNT offset 0x{i:X} " +
                       $"({CntHeader.FieldName(i, null, null)}), original 0x{cnt[i]:X2} vs " +
                       $"resealed 0x{resealed[i]:X2}. Its stored digests do not match its own " +
                       "bytes, so any repair would also rewrite the header signature.";
        return null;
    }

    internal static byte[] Seal(byte[] cnt, IReadOnlyList<CntEntry> given,
                               string contentId, string passcode, Progress? progress = null)
    {
        // DEEP COPY, and this is a behavioural fix over the repair-playgo original, not a
        // tidy-up. That version mutated the CntEntry instances it was handed — DataOffset,
        // DataSize and three derived payloads — so the sequence this command actually needs
        // (fixed-point pre-flight on the untouched table, THEN the real reseal on the modified
        // one) resealed the second time from a table the first pass had already rewritten. The
        // pre-flight then "passed" while silently laundering the input of the real run. Sealing
        // from a private copy makes the call a pure function of its arguments, so the two runs
        // are independent by construction rather than by call-order discipline.
        var physical = given.Select(e => e.Clone()).ToList();
