...
A dry run is what you get unless you pass `--out` or `--in-place`. `--out` refuses to overwrite an existing file, and needs free space beside the target roughly equal to the package's own size (~661 MB on the test package) while it stages the repaired copy before renaming over the target; it writes about 1.32 GB doing so, since the staging file is written twice.

`--in-place` writes into the target's own footprint instead — only the CNT and SI regions actually change, ~64 MB on the 661 MB test package — so it writes about 128 MB in total and needs roughly **64 MB of spare disk**, not none: that is the recovery journal, which holds a copy of the two regions it is about to overwrite and is flushed to disk before the package is touched at all. The repair will not start without room for it. A run that finds a journal left over from an interrupted repair restores the package from it and stops, telling you to re-run once the package is whole again.
