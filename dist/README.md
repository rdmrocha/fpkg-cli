...
A dry run is what you get unless you pass `--out` or `--in-place`. `--out` refuses to overwrite an existing file, and needs free space beside the target roughly equal to the package's own size (~661 MB on the test package) while it stages the repaired copy before renaming over the target; it writes about 661 MB doing so — the payload is staged once, and the rebuilt SI is written at a seek rather than by restaging the file.

`--in-place` writes into the target's own footprint instead — only the CNT and SI regions actually change, ~64 MB on the 661 MB test package — so it writes about 128 MB in total and needs roughly **64 MB of spare disk**, not none — and unlike `--out`'s, that cost does not grow with the package: on a 90 GB package `--in-place` still writes ~64 MB of package data where `--out` writes a full 90 GB copy. The 64 MB is the recovery journal, which holds a copy of the two regions it is about to overwrite and is flushed to disk before the package is touched at all. The repair will not start without room for it. A run that finds a journal left over from an interrupted repair restores the package from it and stops, telling you to re-run once the package is whole again.

Every in-place run prints the journal's path, and leaves a marker file (`<package>.repair-playgo.inprogress`) beside the package naming it, so a later run finds the journal even if you forget `--work-dir` or pass a different one. **Do not delete either file while a repair is in progress or after one was interrupted** — the journal is the package's only way back, and the marker is what stops a later run from mistaking a half-repaired package for a healthy one. If the marker is there but the journal is gone, `fpkg` refuses and says so rather than reporting the package as fine.
